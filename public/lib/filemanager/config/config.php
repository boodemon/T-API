...
function base_path(){
	...
	$ex 	= explode('/', $path);
	if($domain == 'http://ex.saiimog.com' || $domain == 'http://localhost'){
		$base = '/'. $ex[1].'/'.$ex[2];
	}else if(count($ex) >= 5){
		$base = '/'. $ex[1];
	}
...
$config = array(
	...
	*/
	...
	|
	*/
	'upload_dir' => base_path() . 'images/content/',
	...
	*/
	...
	'copy_cut_max_size'                       => 10000,
	// defines file count limit for paste / operation
	// set 'FALSE' for no limit
	'copy_cut_max_count'                      => 20000,
